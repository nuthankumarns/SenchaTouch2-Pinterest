...
(function(global) {
	...
	}, callbacks = [], doc = global.document, head = doc.head, addWindowListener = global.addEventListener, removeWindowListener = global.removeEventListener, storage = global.localStorage, appCache = global.applicationCache, jsonParse = JSON.parse, a = doc
			.createElement("a"), documentLocation = doc.location, documentUri = documentLocation.origin
			+ documentLocation.pathname + documentLocation.search, manifestFile = "app.json", isRefreshing = false, activeManifest;
	...
	function Manifest(manifest) {
		...
		var applicationId = manifest.id, key = getManifestStorageKey(applicationId), assetMap = {};
	...
	function requestXhr(uri, onSuccess, onFailure) {
		...
		try {
			...
			xhr.onreadystatechange = function() {
				if (xhr.readyState == 4) {
					var status = xhr.status, content = xhr.responseText;
		...
		if (!isShared) {
			...
			onSuccess = function(content) {
				...
				if (remoteChecksumBlock !== "/*" + version + "*/") {
					if (confirm("Requested: '" + asset.uri
							+ "' with checksum: " + version + " but received: "
							+ remoteChecksumBlock.substring(2, version.length)
	...
	function onMessage(e) {
		var data = e.data, sourceWindow = e.source.window, i, ln, callback, iframe;
	...
	function blink(currentManifest) {
		var currentAssets = currentManifest.assets, assetsCount = currentAssets.length, newManifest;
		...
		function onReady() {
			var updatingAssets = [], appCacheReady = false, onAppCacheIdle = function() {
			...
			function checkForUpdate() {
				...
				requestXhr(manifestFile, function(manifestContent) {
					...
					assets.forEach(function(asset) {
								currentAsset = currentManifest
										.getAsset(asset.uri);
								if (!currentAsset
										|| asset.version !== currentAsset.version) {
					...
					updatingAssets.forEach(function(asset) {
						var currentAsset = currentManifest.getAsset(asset.uri), path = asset.path, update = asset.update;
						function updateFull() {
							requestAsset(asset, function(content) {
										onAssetUpdated(asset, content)
									})
						}
						if (!currentAsset || !update
								|| retrieveAsset(asset) === null
								|| update != "delta") {
							updateFull()
						} else {
							requestXhr("deltas/" + path + "/"
											+ currentAsset.version + ".json",
									function(content) {
										try {
											...
											log("Malformed delta content received for "
													+ asset.uri)
										}
									}, updateFull)
						}
